...
for idx, stockitem in stockitems.iterrows():
    count +=1
    query = 'SELECT * FROM Table_KOSPI WHERE code = "{}" limit 3000'.format(stockitem['code'])
    df = pd.read_sql(query, con=con)
    
    df_close = df[['close']]
    df_all = df.drop(['index', 'code', 'section', 'date'], axis=1) #분석에 필요없는 행 제거
