...
for i in range(0, int(len(rows)/unit)):
    ...
    table1_1.to_sql('Table1_KOSDAQ', con, chunksize = 1000, if_exists='append')

print("남은 데이터를 추가 저장합니다.")

i+=1
table1_2 = table1.iloc[i*unit:len(rows)]
table1_2.to_sql('Table1_KOSDAQ', con, chunksize = 1000, if_exists='append')
